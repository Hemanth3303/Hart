...
namespace Hart {
	std::string FileManager::ReadStringFromFile(const std::string& fileName) {
		...
		if (!FileExists(fileName)) {
			HART_ENGINE_ERROR(LogSource::EngineUtils, "Could not find file named ", fileName, "\n\t\t\tIs the file name and/or file path correct?");
		}
		else {
			HART_ENGINE_INFO(LogSource::EngineUtils, "Opening file: ", fileName);
	...
	void FileManager::WriteStringToFile(const std::string& data, const std::string& fileName, const std::string& directory) {
		...
		if (!std::filesystem::is_directory(directory)) {
			HART_ENGINE_INFO(LogSource::EngineUtils, "Directory ", directory, " doesn't exist. Creating it");
		...
		HART_ENGINE_INFO(LogSource::EngineUtils, "Writing to file " , outFileName);
	...
	uintmax_t FileManager::GetFileSizeInBytes(const std::string& filepath) {
		if (!FileExists(filepath)) {
			HART_ENGINE_ERROR(LogSource::EngineUtils, filepath, " not found");
			return 0;
		}
		...
		if (errorCode) {
			HART_ENGINE_ERROR(LogSource::EngineUtils, filepath, " could not determine file size\n", errorCode.message());
			return 0;
		}
